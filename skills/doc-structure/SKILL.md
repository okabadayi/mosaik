...
- **Relocate — the default for any decision carrying substantial detail.** **First check whether that detail already lives in an existing doc** (a `_reference.md`, `architecture.md`, a runbook). **If it does, you create nothing — the CHANGELOG entry just becomes a stub pointing at that existing doc.** This is the common case in a repo with real reference docs; reach for it before writing anything. Only detail that lives *nowhere else* needs a home — put it in the closest existing topic doc, or a new `docs/reference/<topic>.md` only if none fits. Either way the CHANGELOG keeps a dated stub (decision + why + `Reference:`); **nothing is deleted — it's rehomed where it belongs, more findable, not less.** Don't duplicate detail that's already homed, and don't spawn one doc per entry.
