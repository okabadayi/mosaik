...
2. **CHANGELOG Decision Log** — walk per-entry. For each entry past the threshold (>~10 lines body / multiple sub-findings / implementation-detail dump), apply the triage from `skills/doc-structure/SKILL.md § CHANGELOG Format`. **The discriminator is "is this stored anywhere else?", never "is it long?":**
   - **Relocate is the *default*.** **First check whether the detail already lives in an existing doc** (`_reference.md`, `architecture.md`, runbook): if so, **create nothing — the entry becomes a stub pointing there.** This is the common case in a repo with real reference docs. Only detail that exists *nowhere else* needs a home — the closest existing topic doc, or a new `docs/reference/<topic>.md` if none fits. The CHANGELOG keeps a dated stub (decision + why + `Reference:`); **nothing is deleted.** Don't duplicate already-homed detail; don't spawn one doc per entry.
